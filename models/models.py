...
#        n,h,w,c=img_g.get_shape().as_list()
    ...
    def build_model(self):
        ...
        loss_p=tf.reduce_mean((self.diff2(gt_i,0)-self.diff2(x_unwrap[2],0))**2)+tf.reduce_mean((self.diff2(gt_i,1)-self.diff2(x_unwrap[2],1))**2)
        tf.summary.scalar('loss_p' , 2.5*loss_p)
        self.loss_total+=loss_p*2.5
        for i in range(self.batch_size):
            self.ssim= self.ssim+tf.image.ssim(img_gt[i,:,:,:], x_unwrap[self.n_levels-1][i,:,:,:], max_val=1.0)
            self.psnr= self.psnr+tf.image.psnr(img_gt[i,:,:,:], x_unwrap[self.n_levels-1][i,:,:,:], max_val=1.0)
...
#        sess=tf.Session()
        ...
        for step in range(sess.run(global_step), self.max_steps+1):
            ...
            if step % 5 == 0:
                ...
                print(format_str % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), step, loss_total_val, psnr,t_psnr/cnt,t_ssim/cnt,examples_per_sec, sec_per_batch))
    ...
    def test(self, height, width, input_path, output_path):
        ...
        self.load(sess, self.train_dir+'/checkpoints/', step=675000)
